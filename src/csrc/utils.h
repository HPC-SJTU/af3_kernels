...
class VLAPtr {
	public:
	VLAPtr(T* data_, const index_t (&sizes)[N])
		: data_(data_) {
	...
	VLAPtr(T* data_, const index_t (&strides_)[N], void* dymmy)
		: data_(data_) {
...
class VLAPtr<T, 1, int64_t> {
	...
	VLAPtr(T* data_, const index_t (&sizes)[1])
		: data_(data_) {
		strides[0] = sizes[0];
	}

	// uses dummy pointer to distinguish from the constructor with sizes
	VLAPtr(T* data_, const index_t (&strides_)[1], void* dymmy)
		: data_(data_) {
...
inline T* pt_get_data_ptr(at::Tensor t) {
	...
	if(c10::CppTypeToScalarType<T>::value != t.dtype()) {
		if(t.is_quantized() && (std::is_same<T, uint8_t>::value || std::is_same<T, int8_t>::value)) {
			return t.data_ptr<T>();
		}
...
template<typename T, std::size_t N, typename index_t = int64_t>
VLAPtr<T, N, index_t> GetVLAPtr(at::Tensor t, const index_t (&list)[N]) {
	return VLAPtr<T, N, index_t>(pt_get_data_ptr<T>(t), list);
}
